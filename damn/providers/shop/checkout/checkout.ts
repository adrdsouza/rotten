...
export const processNMIPayment = async (cardData: {
	cardNumber: string;
	expiryDate: string;
	cvv: string;
}) => {
	try {
		console.log('[NMI] Processing payment with card data...');
		
		// Add payment to order with NMI method
		...
		if (paymentResult && 'code' in paymentResult) {
			return paymentResult;
		} else {
			throw new Error('Payment failed - no order code received');
		}
	} catch (error) {
		console.error('[NMI] Payment processing failed:', error);
		throw error;
	}
};
