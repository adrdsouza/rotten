...
export const refreshCartStock = $(async (cartState: CartContextState) => {
  ...
  try {
    ...
    // Update lastStockValidation with fresh stock data
    const stockValidation = await LocalCartService.validateCartStock();
    cartState.lastStockValidation = stockValidation;
...
export const convertLocalCartToVendureOrder = $(async (cartState: CartContextState) => {
  ...
  try {
    ...
    const stockValidation = await validateLocalCart(cartState);
...
export const validateLocalCart = $(async (cartState: CartContextState) => {
  const stockValidation = await LocalCartService.validateCartStock();
  cartState.lastStockValidation = stockValidation;
  const errors: string[] = [];
  for (const variantId in stockValidation) {
    if (!stockValidation[variantId].success) {
      errors.push(stockValidation[variantId].error || 'Unknown error');
    }
  }
  return {
