...
export const onGet: RequestHandler = async ({ send, headers }) => {
  try {
    ...
    console.error('[sitemap.xml] Backend fetch failed, returning fallback sitemap index');
...
<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  <sitemap>
    <loc>https://damneddesigns.com/sitemap-main.xml</loc>
    <lastmod>${timestamp}</lastmod>
  </sitemap>
  <sitemap>
    <loc>https://damneddesigns.com/sitemap-products.xml</loc>
    <lastmod>${timestamp}</lastmod>
  </sitemap>
  <sitemap>
    <loc>https://damneddesigns.com/sitemap-collections.xml</loc>
    <lastmod>${timestamp}</lastmod>
  </sitemap>
</sitemapindex>`;
