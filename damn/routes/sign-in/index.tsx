			...
			if (registrationResult?.__typename === 'Success' && registrationResult.success === true) {
				...
				if (registrationResult && registrationResult.__typename !== 'Success') {
					...
					if (registrationResult.errorCode === 'EMAIL_ADDRESS_CONFLICT_ERROR' ||
						msg.includes('email') || msg.includes('already') || msg.includes('exists')) {
					...
					<div class={`${activeTab.value === 'signin' ? 'block' : 'hidden'} md:block`}>
						<div class="bg-[#f5f5f5] rounded-2xl p-8 shadow-sm">
							...
							<div class="space-y-6">
								...
								<div>
									<button
										class="w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#d42838] hover:bg-black focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-colors cursor-pointer"
										onClick$={login}
									>
										Sign In
									</button>
