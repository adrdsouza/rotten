...
export const CheckoutAddresses = component$<CheckoutAddressesProps>(({ onAddressesSubmitted$ }) => {
  ...
  useVisibleTask$(({ track }) => {
    ...
    if (appState.shippingAddress?.streetLine1) {
      ...
      // Mark state/province as touched if it has a value from login
      if (appState.shippingAddress?.province) {
        // Note: State validation is handled by AddressForm component
        // This ensures the field is marked as touched for UI consistency
      }

      // Mark postal code as touched if it has a value from login
      if (appState.shippingAddress?.postalCode) {
        // Note: Postal code validation is handled by AddressForm component
        // This ensures the field is marked as touched for UI consistency
      }
