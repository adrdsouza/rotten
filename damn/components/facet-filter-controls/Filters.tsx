	...
	({
		...
		return (
			<div>
									...
									<div class="space-y-4">
										{facet.values.map((value) => (
											<div key={value.id} class="flex items-center cursor-pointer">
												<label class="text-sm text-gray-600 cursor-pointer flex items-center">
													<input
				...
				{showMenu && (
					<div class="relative z-9999 lg:hidden">
						<div class="fixed inset-0 bg-black/25 opacity-100 z-9998"></div>
						<div class="fixed inset-0 flex z-9999">
							<div class="ml-auto relative max-w-xs w-full h-full bg-white shadow-xl py-4 pb-12 flex flex-col overflow-y-auto translate-x-0">
								<div class="px-4 flex items-center justify-between">
									...
									<button
										type="button"
										class="-mr-2 w-10 h-10 bg-white p-2 flex items-center justify-center text-gray-400 hover:text-gray-600"
										onClick$={$(async () => {
											onToggleMenu$();
										})}
									>
														...
														{facet.values.map((value) => (
															<div key={value.id} class="flex items-center cursor-pointer">
																<input
																	...
																	onClick$={() => onFilterChange$(value.id)}
																/>
																<label class="ml-3 min-w-0 flex-1 text-gray-500 cursor-pointer">
																	{value.name}
																</label>
															</div>
														))}
