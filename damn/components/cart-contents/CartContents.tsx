...
export default component$<{
	...
	const isOrderConfirmation = !!order; // If order prop is passed, we're on order confirmation
								...
								<div class="flex justify-between items-start mb-1">
											...
											{(item as any).isPreOrder && (
												<span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800 mt-1">
													...
													{(item as any).shipDate && (
														<span class="ml-1 text-red-600">• Ships {new Date((item as any).shipDate as any).toLocaleDateString()}</span>
													)}
												</span>
											)}
									...
									<div class="text-right">
										<Price
											...
											preOrderPrice={(item as any).preOrderPrice}
										/>
									</div>
								</div>
							...
							<div class="ml-4 flex-1 flex flex-col justify-center w-full">
								...
								<div class="flex justify-between items-start mb-1">
									...
									<div class="flex flex-col">
										...
										{/* Show quantity above pre-order badge on order confirmation */}
										{isOrderConfirmation && (line as any).productVariant?.customFields?.preOrderPrice && (
											<div class="mt-1 mb-1">
												<span class="font-medium text-gray-700 text-sm">Qty: {line.quantity}</span>
											</div>
										)}
										{(line as any).productVariant?.customFields?.preOrderPrice && (
											<span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800 mt-1">
												PRE-ORDER
												{(line as any).productVariant?.customFields?.shipDate && (
													<span class="ml-1 text-red-600">• Ships {new Date((line as any).productVariant.customFields.shipDate as any).toLocaleDateString()}</span>
												)}
											</span>
										)}
								...
								{/* Bottom row: quantity and remove button */}
										<div class="flex items-center justify-between mt-2">
											{/* Quantity selector with no label - only show if not order confirmation with pre-order badge */}
											{!(isOrderConfirmation && (line as any).productVariant?.customFields?.preOrderPrice) && (
												<div>
													{isInEditableUrl && !isOrderConfirmation ? (
														<select
														...
														<span class="font-medium text-gray-700">Qty: {line.quantity}</span>
													)}
												</div>
											)}
