...
export const useHasPreOrder = () => {
  ...
  try {
    ...
    has.value = items.some((it) => (it as any)?.productVariant?.customFields?.preOrderPrice);
  } catch {
    has.value = false;
  }
  return has;
...
export const loadCartIfNeeded = $((cartState: CartContextState) => {
  if (!cartState.hasLoadedOnce) {
    try {
      ...
      // Ensure cart is in local mode if it has items
      if (cartState.localCart.items.length > 0) {
