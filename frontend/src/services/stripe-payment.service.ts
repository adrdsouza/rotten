...
export class StripePaymentService {
  ...
  async createPaymentIntent(estimatedTotal: number, currency = 'usd'): Promise<PaymentIntentResult> {
    ...
    try {
      ...
      const clientSecret = response.data.createPreOrderStripePaymentIntent;
  ...
  async settlePayment(paymentIntentId: string): Promise<SettlementResult> {
    try {
      ...
      const success = response.data.settleStripePayment;
  ...
  async getPaymentIntentStatus(paymentIntentId: string): Promise<any> {
    try {
      ...
      return JSON.parse(response.data.getPaymentIntentStatus);
    } catch (error) {
      console.error('❌ Failed to get PaymentIntent status:', error);
