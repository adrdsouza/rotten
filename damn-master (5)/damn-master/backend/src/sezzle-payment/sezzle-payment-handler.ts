...
export const sezzlePaymentHandler = new PaymentMethodHandler({
  ...
  createPayment: async (ctx, order, amount, args, _metadata): Promise<CreatePaymentResult> => {
    try {
      ...
      // Hydrate the order with surcharges relation to ensure taxSummary property is accessible
      await entityHydrator.hydrate(ctx, order, { relations: ['surcharges'] });
      Logger.info(`[Sezzle] Order hydrated with surcharges relation for ${order.code}`, 'SezzlePaymentHandler');
  ...
  settlePayment: async (ctx, order, payment, args): Promise<SettlePaymentResult | SettlePaymentErrorResult> => {
    try {
      // Hydrate the order with lines and surcharges relations to ensure discounts and taxSummary properties are accessible
      // This is required because PaymentService.settlePayment() internally accesses order.discounts and we access taxSummary
      await entityHydrator.hydrate(ctx, order, { relations: ['lines', 'surcharges'] });
      Logger.info(`[Sezzle] Order hydrated with lines and surcharges relations for ${order.code}`, 'SezzlePaymentHandler');
