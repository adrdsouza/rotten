...
export const nmiPaymentHandler = new PaymentMethodHandler({
  ...
  createPayment: async (ctx, order, amount, args, metadata): Promise<CreatePaymentResult> => {
    ...
    const { testMode, strictAvsCvv: _strictAvsCvv } = args;
    ...
    Logger.info('Received NMI response for payment', 'NmiPaymentHandler');
      ...
      const _isDeclined = responseData.response === '2' || responseData.response === '3';
      const _avsMatch = responseData.avsresponse && !['N', 'U', 'R'].includes(responseData.avsresponse);
      const _cvvMatch = responseData.cvvresponse && responseData.cvvresponse !== 'N';
