...
import { type CreateCustomerInput } from '~/generated/graphql-shop';
import { CheckoutAddresses } from '~/components/checkout/CheckoutAddresses';
import { CheckoutAddressProvider } from '~/contexts/CheckoutAddressContext';
import { createSEOHead } from '~/utils/seo';
import Payment from '~/components/payment/Payment';
import { useLocalCart, refreshCartStock, loadCartIfNeeded, useHasPreOrder } from '~/contexts/CartContext';
...
const CheckoutContent = component$(() => {
  ...
  const hasPreOrder = useHasPreOrder();
  ...
  const placeOrder = $(async () => {
    ...
    try {
      ...
      // CRITICAL: Set customer information for the order FIRST
      try {
        const customerInput: CreateCustomerInput = {
          ...
          phoneNumber: appState.shippingAddress.phoneNumber || appState.customer.phoneNumber || ''
        };
        
        const customerResult = await setCustomerForOrderMutation(customerInput);
        if (customerResult && '__typename' in customerResult && customerResult.__typename === 'Order') {
          appState.activeOrder = customerResult as any;
          console.log('✅ Customer information set successfully for order');
        } else {
          throw new Error('Failed to set customer information for order');
        }
      } catch (customerError) {
        console.error('❌ Failed to set customer for order:', customerError);
        throw new Error('Failed to set customer information. Please check your details and try again.');
      }

      // Use CheckoutOptimizationService for efficient address and shipping method processing
      try {
        // Transform ShippingAddress to AddressInput format
        ...
        // Update the active order with the optimized result
        ...
        // Fallback to manual address form submission
      ...
      // Now that we have customer info, address, and shipping method, transition to ArrangingPayment
              ...
              <div class="order-2 lg:order-1 mb-8 lg:mb-0">
                <div class="sticky top-4">
                  <div class="bg-[#f5f5f5] rounded-2xl border border-gray-200/50 shadow-lg backdrop-blur-sm overflow-hidden transition-all duration-300 hover:shadow-xl">
                    ...
                    {hasPreOrder.value && (
                      <div class="bg-amber-50 border border-amber-200 rounded-lg mx-4 mb-4 p-4">
                        <div class="flex items-start">
                          ...
                          <div class="ml-3">
                            <h3 class="text-sm font-medium text-amber-800">Pre-Order Items Notice</h3>
                            <div class="mt-2 text-sm text-amber-700">
                              <p>Your order contains pre-order items. Your entire order will ship together when the last pre-order item becomes available. See individual ship dates above for timing.</p>
                            </div>
                          </div>
                        </div>
                      </div>
