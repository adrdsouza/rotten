	...
	try {
		...
		if (!result || !result.product) {
			...
			if (!product) {
				...
				throw fail(404, { message: `Product not found: ${slug}` });
			}
			result = {
				product,
				source: 'fallback',
		...
		try {
			...
			// If all fallbacks fail, return a proper 404 response
			console.warn(`Product not found after all fallbacks: ${slug}`);
			throw fail(404, { message: `Product not found: ${slug}` });
		}
	}
