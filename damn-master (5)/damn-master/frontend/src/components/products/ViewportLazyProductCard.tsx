...
export default component$<ViewportLazyProductCardProps>(
	({ productAsset, productName, slug, priceWithTax, inStock, productId }) => {
		...
		// 🚀 OPTIMIZED: Use global intersection observer for lazy loading (only if not cached)
		useVisibleTask$(({ cleanup }) => {
			// Skip intersection observer if image is already cached
			if (isLikelyCached || !elementRef.value) return;

			// Use global intersection observer for better performance
			globalIntersectionObserver.observe(
				...
				{
					rootMargin: '300px', // Start loading 300px before the element becomes visible
					threshold: 0.1,
