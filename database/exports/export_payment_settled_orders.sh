...
# Script to export orders in PaymentSettled state only
# These are orders that have been paid and are ready to ship
# Created: $(date +%Y-%m-%d)

# Set the output filename with today's date
OUTPUT_FILE="payment_settled_orders_export_$(date +%Y-%m-%d).csv"

echo "Exporting PaymentSettled orders to $OUTPUT_FILE..."
echo "This includes only orders that have been paid and are ready to ship."
...
if [ $? -eq 0 ]; then
    ...
    wc -l "$OUTPUT_FILE"
    echo ""
    echo "First few lines of the export:"
    head -3 "$OUTPUT_FILE"
else
    echo "Export failed!"
    exit 1
...
echo ""
echo "Note: This export contains only orders in 'PaymentSettled' state."
echo "These are orders that have been paid and are ready to ship."