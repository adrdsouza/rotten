...
export const getActiveOrderQuery = async () => {
	return shopSdk.activeOrder(undefined).then((res: ActiveOrderQuery) => {
		console.log('[getActiveOrderQuery] Raw response:', JSON.stringify(res, null, 2));
		console.log('[getActiveOrderQuery] activeOrder field:', res.activeOrder);
		console.log('[getActiveOrderQuery] activeOrder state:', res.activeOrder?.state);
...
export const addItemToOrderMutation = async (productVariantId: string, quantity: number) => {
	return shopSdk
		...
		.then((res: AddItemToOrderMutation) => {
			console.log('[addItemToOrderMutation] Raw response:', JSON.stringify(res, null, 2));
			console.log('[addItemToOrderMutation] addItemToOrder field:', res.addItemToOrder);
			if (res.addItemToOrder && typeof res.addItemToOrder === 'object' && '__typename' in res.addItemToOrder) {
				console.log('[addItemToOrderMutation] Result type:', res.addItemToOrder.__typename);
				if (res.addItemToOrder.__typename === 'Order') {
					console.log('[addItemToOrderMutation] Order state:', (res.addItemToOrder as any).state);
					console.log('[addItemToOrderMutation] Order code:', (res.addItemToOrder as any).code);
				}
			}
